...
def replace_words_in_query(query_string, column_name, new_words):
    ...
    contains_indices = [
        ...
        if re.match(
            rf"^{re.escape(column_name)}(?:\.fillna\(''\))?\.str\.contains\('", part
        )
    ]
...
def make_string_fuzzy_regex(name):
    ...
    for char in name.lower():
        if char not in ["*", "?", "|", "&", "(", ")"]:
            ...
            if char not in substitutions:
                ...
                fuzzy_chars.append(
                    f"{new_char}.?"
                )  # Add optional character after each letter
        else:
    ...
    fuzzy_pattern = (
        f"{fuzzy_pattern[:-2]}"  # Remove last .? to prevent too much fuzziness
    )
...
def split_alphanum(input_string):
    ...
    step2_result = re.sub(
        r"(?<=\d)(?=[a-zA-Z])|(?<=[a-zA-Z])(?=\d)", ".*", step1_result
    )
